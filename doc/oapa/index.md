...
- Automatic correction loop in Ekos (INDI >= 2.2.0, KStars >= 3.8.2)
...
2. Enter a known step in **Manual Adjustment**, e.g. `0.1` degrees, with a rough
   calibration value set.
3. Refresh the solution and note how much the error actually changed.
4. New value = old value x requested change / measured change.
...
`tests/fake_oapa_firmware.py` emulates firmware 1.2.2 on a pseudo-terminal, and
`tests/pac_contract_test.sh` runs the PAC contract against it (driver currents on
connection and on change, calibration refusal, two-axis move and completion, per-axis
speed, reverse, sub-step request, abort, stall, old firmware).
